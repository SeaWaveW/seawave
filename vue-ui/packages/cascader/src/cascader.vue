<template>
  <div
    ...
    <sw-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="sw-input__icon sw-icon-circle-close"
    ...
    <div v-if="multiple" class="sw-cascader__tags">
      <sw-tag
      ...
      <input
        ...
        type="text"
        class="sw-cascader__search-input"
    ...
    <transition name="sw-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['sw-popper', 'sw-cascader__dropdown', popperClass]">
        <sw-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></sw-cascader-panel>
        <sw-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="sw-cascader__suggestion-panel"
          view-class="sw-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'sw-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="sw-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="sw-cascader__empty-text">{{ t('sw.cascader.noMatch') }}</li>
          </slot>
        </sw-scrollbar>
      </div>
    </transition>
...
import SwCascaderPanel from 'seawave-ui/packages/cascader-panel';
import AriaUtils from 'seawave-ui/src/utils/aria-utils';
import { t } from 'seawave-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'seawave-ui/src/utils/util';
import { isUndefined, isFunction } from 'seawave-ui/src/utils/types';
import { isDef } from 'seawave-ui/src/utils/shared';
...
export default {
  ...
  props: {
    ...
    placeholder: {
      type: String,
      default: () => t('sw.cascader.placeholder')
    },
    disabled: Boolean,
    clearable: Boolean,
    filterable: Boolean,
  ...
  computed: {
    realSize() {
      const _swFormItemSize = (this.swFormItem || {}).swFormItemSize;
      return this.size || _swFormItemSize || (this.$ELEMENT || {}).size;
    },
    tagSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.sw-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.sw-cascader-menu');
          firstNode = firstMenu.querySelector('.sw-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.sw-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.sw-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.sw-cascader__suggestion-list');
