...
import {
  ...
  isEmpty,
  valueEquals
} from 'seawave-ui/src/utils/util';
...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.sw-cascader-node[tabindex="-1"]');
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.sw-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.sw-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    */
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.sw-scrollbar__wrap');
          const activeNode = menuElement.querySelector('.sw-cascader-node.is-active') ||
            menuElement.querySelector('.sw-cascader-node.in-active-path');
