...
export default {
  ...
  methods: {
    ...
    renderEmptyText(h) {
      return (
        <div class="sw-cascader-menu__empty-text">{ this.t('sw.cascader.noData') }</div>
      );
    },
    renderNodeList(h) {
      const { menuId } = this;
      ...
      return [
        ...nodes,
        isHoverMenu ? <svg ref='hoverZone' class='sw-cascader-menu__hover-zone'></svg> : null
      ];
    }
  },
  ...
  render(h) {
    ...
    return (
      <el-scrollbar
        ...
        role="menu"
        id={ menuId }
        class="sw-cascader-menu"
        wrap-class="sw-cascader-menu__wrap"
        view-class={{
          'sw-cascader-menu__list': true,
          'is-empty': isEmpty
        }}
        { ...events }>
