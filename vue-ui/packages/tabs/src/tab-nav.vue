  ...
  export default {
    ...
    render(h) {
      ...
      const scrollBtn = scrollable
        ? [
          <span class={['sw-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="sw-icon-arrow-left"></i></span>,
          <span class={['sw-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="sw-icon-arrow-right"></i></span>
        ] : null;
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        const btnClose = closable
          ? <span class="sw-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;
      ...
      return (
        <div class={['sw-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${ this.rootTabs.tabPosition }`]}>
          {scrollBtn}
          <div class={['sw-tabs__nav-scroll']} ref="navScroll">
            <div
              class={['sw-tabs__nav', `is-${ this.rootTabs.tabPosition }`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
              ref="nav"
              style={navStyle}
              role="tablist"
