<template>
  <div class="sw-calendar">
    <div class="sw-calendar__header">
      <div class="sw-calendar__title">
        {{ i18nDate }}
      </div>
      <div
        class="sw-calendar__button-group"
        ...
        <sw-button-group>
          <sw-button
            ...
            {{ t('sw.datepicker.prevMonth') }}
          </sw-button>
          <sw-button
            type="plain"
            size="mini"
            @click="selectDate('today')">
            {{ t('sw.datepicker.today') }}
          </sw-button>
          <sw-button
            type="plain"
            size="mini"
            @click="selectDate('next-month')">
            {{ t('sw.datepicker.nextMonth') }}
          </sw-button>
        </sw-button-group>
      </div>
    </div>
    <div
      class="sw-calendar__body"
      v-if="validatedRange.length === 0"
      key="no-range">
      <date-table
        :date="date"
...
import fecha from 'seawave-ui/src/utils/date';
import SwButton from 'seawave-ui/packages/button';
import SwButtonGroup from 'seawave-ui/packages/button-group';
import DateTable from './date-table';
import { validateRangeInOneMonth } from 'seawave-ui/src/utils/date-util';
...
export default {
  ...
  methods: {
    ...
    rangeValidator(date, isStart) {
      ...
      if (date.getDay() !== expected) {
        console.warn('[SeawaveCalendar]', message, 'Invalid range will be ignored.');
        return false;
      }
      return true;
    }
  ...
  computed: {
    ...
    i18nDate() {
      ...
      return `${year} ${this.t('sw.datepicker.year')} ${this.t('sw.datepicker.month' + month)}`;
    },
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn('[SeawaveCalendar]end time should be greater than start time');
          return [];
        }
        ...
        if (!validateRangeInOneMonth(startDay, end)) {
          console.warn('[SeawaveCalendar]start time and end time interval must not exceed two months');
          return [];
        }
        // 第一个月的时间范围
        data.push([
