...
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, getI18nSettings, validateRangeInOneMonth } from 'seawave-ui/src/utils/date-util';
...
export default {
  ...
  methods: {
    ...
    cellRenderProxy({ text, type }) {
      let render = this.swCalendar.$scopedSlots.dateCell;
  ...
  computed: {
    ...
    formatedToday() {
      return this.swCalendar.formatedToday;
    },
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'sw-calendar-table__row': true,
                'sw-calendar-table__row--hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {
