<script>
  ...
  export default {
    ...
    computed: {
      ...
      menuTransitionName() {
        return this.rootMenu.collapse ? 'sw-zoom-in-left' : 'sw-zoom-in-top';
      },
      opened() {
      ...
      isFirstLevel() {
        ...
        while (parent && parent !== this.rootMenu) {
          if (['SwSubmenu', 'SwMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
            isFirstLevel = false;
            break;
          } else {
    ...
    methods: {
      ...
      handleClick() {
        ...
        this.dispatch('SwMenu', 'submenu-click', this);
      },
      handleMouseenter(event, showTimeout = this.showTimeout) {
        ...
        this.dispatch('SwSubmenu', 'mouse-enter-child');
      ...
      handleMouseleave(deepDispatch = false) {
        ...
        this.dispatch('SwSubmenu', 'mouse-leave-child');
        clearTimeout(this.timeout);
        this.timeout = setTimeout(() => {
        ...
        if (this.appendToBody && deepDispatch) {
          if (this.$parent.$options.name === 'SwSubmenu') {
    ...
    render(h) {
      ...
      const popupMenu = (
        <transition name={menuTransitionName}>
          <div
            ref="menu"
            v-show={opened}
            class={[`sw-menu--${mode}`, popperClass]}
            ...
            <ul
              role="menu"
              class={['sw-menu sw-menu--popup', `sw-menu--popup-${currentPlacement}`]}
      ...
      ) ? 'sw-icon-arrow-down' : 'sw-icon-arrow-right';
      ...
      return (
        ...
        >
          ...
          >
            {$slots.title}
            <i class={[ 'sw-submenu__icon-arrow', submenuTitleIcon ]}></i>
          </div>
