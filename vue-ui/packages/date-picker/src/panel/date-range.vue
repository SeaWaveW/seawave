<template>
  <transition name="sw-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="sw-picker-panel sw-date-range-picker sw-popper"
      :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="sw-picker-panel__body-wrapper">
        <slot name="sidebar" class="sw-picker-panel__sidebar"></slot>
        <div class="sw-picker-panel__sidebar" v-if="shortcuts">
          <button
            type="button"
            class="sw-picker-panel__shortcut"
            v-for="(shortcut, key) in shortcuts"
            :key="key"
            @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="sw-picker-panel__body">
          <div class="sw-date-range-picker__time-header" v-if="showTime">
            <span class="sw-date-range-picker__editors-wrap">
              <span class="sw-date-range-picker__time-picker-wrap">
                <sw-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="minInput"
                  :placeholder="t('sw.datepicker.startDate')"
                  class="sw-date-range-picker__editor"
                  :value="minVisibleDate"
                  @input="val => handleDateInput(val, 'min')"
                  @change="val => handleDateChange(val, 'min')" />
              </span>
              <span class="sw-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
                <sw-input
                  size="small"
                  class="sw-date-range-picker__editor"
                  :disabled="rangeState.selecting"
                  :placeholder="t('sw.datepicker.startTime')"
            ...
            <span class="sw-icon-arrow-right"></span>
            <span class="sw-date-range-picker__editors-wrap is-right">
              <span class="sw-date-range-picker__time-picker-wrap">
                <sw-input
                  size="small"
                  class="sw-date-range-picker__editor"
                  :disabled="rangeState.selecting"
                  :placeholder="t('sw.datepicker.endDate')"
                  :value="maxVisibleDate"
                  :readonly="!minDate"
                  @input="val => handleDateInput(val, 'max')"
                  @change="val => handleDateChange(val, 'max')" />
              </span>
              <span class="sw-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
                <sw-input
                  size="small"
                  class="sw-date-range-picker__editor"
                  :disabled="rangeState.selecting"
                  :placeholder="t('sw.datepicker.endTime')"
          ...
          <div class="sw-picker-panel__content sw-date-range-picker__content is-left">
            <div class="sw-date-range-picker__header">
              <button
                type="button"
                @click="leftPrevYear"
                class="sw-picker-panel__icon-btn sw-icon-d-arrow-left"></button>
              <button
                type="button"
                @click="leftPrevMonth"
                class="sw-picker-panel__icon-btn sw-icon-arrow-left"></button>
              <button
                type="button"
                @click="leftNextYear"
                v-if="unlinkPanels"
                ...
                class="sw-picker-panel__icon-btn sw-icon-d-arrow-right"></button>
              <button
                type="button"
                @click="leftNextMonth"
                v-if="unlinkPanels"
                ...
                class="sw-picker-panel__icon-btn sw-icon-arrow-right"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="sw-picker-panel__content sw-date-range-picker__content is-right">
            <div class="sw-date-range-picker__header">
              <button
                type="button"
                @click="rightPrevYear"
      ...
      <div class="sw-picker-panel__footer" v-if="showTime">
        <sw-button
          size="mini"
          type="text"
          class="sw-picker-panel__link-btn"
          @click="handleClear">
          {{ t('sw.datepicker.clear') }}
        </sw-button>
        <sw-button
          plain
          size="mini"
...
<script type="text/babel">
  import {
    ...
    extractDateFormat,
    extractTimeFormat
  } from 'seawave-ui/src/utils/date-util';
  ...
  export default {
    ...
    computed: {
      ...
      leftLabel() {
        return this.leftDate.getFullYear() + ' ' + this.t('sw.datepicker.year') + ' ' + this.t(`sw.datepicker.month${ this.leftDate.getMonth() + 1 }`);
      },

      rightLabel() {
        return this.rightDate.getFullYear() + ' ' + this.t('sw.datepicker.year') + ' ' + this.t(`sw.datepicker.month${ this.rightDate.getMonth() + 1 }`);
      },
    ...
    methods: {
      ...
      handleRangePick(val, close = true) {
        ...
        // workaround for https://github.com/SeaWaveW/seawave-ui/issues/7539, should remove this block when we don't have to care about Chromium 55 - 57
        setTimeout(() => {
          this.maxDate = maxDate;
          this.minDate = minDate;
        }, 10);
    ...
    components: { TimePicker, DateTable, SwInput, SwButton }
  };
</script>
