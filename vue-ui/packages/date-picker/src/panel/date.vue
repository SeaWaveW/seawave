<template>
  <transition name="sw-zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div
      v-show="visible"
      class="sw-picker-panel sw-date-picker sw-popper"
      :class="[{
      ...
      <div class="sw-picker-panel__body-wrapper">
        ...
        <div class="sw-picker-panel__body">
          <div class="sw-date-picker__time-header" v-if="showTime">
            <span class="sw-date-picker__editor-wrap">
              <sw-input
                :placeholder="t('sw.datepicker.selectDate')"
                :value="visibleDate"
                size="small"
                @input="val => userInputDate = val"
                @change="handleVisibleDateChange" />
            </span>
            <span class="sw-date-picker__editor-wrap" v-clickoutside="handleTimePickClose">
              <sw-input
                ref="input"
                @focus="timePickerVisible = true"
                :placeholder="t('sw.datepicker.selectTime')"
                :value="visibleTime"
                size="small"
          ...
          <div
            class="sw-date-picker__header"
            :class="{ 'sw-date-picker__header--bordered': currentView === 'year' || currentView === 'month' }"
            v-show="currentView !== 'time'">
            <button
              type="button"
              @click="prevYear"
              :aria-label="t(`sw.datepicker.prevYear`)"
              class="sw-picker-panel__icon-btn sw-date-picker__prev-btn sw-icon-d-arrow-left">
            </button>
            <button
              type="button"
              @click="prevMonth"
              v-show="currentView === 'date'"
              :aria-label="t(`sw.datepicker.prevMonth`)"
              class="sw-picker-panel__icon-btn sw-date-picker__prev-btn sw-icon-arrow-left">
            </button>
            <span
              @click="showYearPicker"
              role="button"
              class="sw-date-picker__header-label">{{ yearLabel }}</span>
            <span
              @click="showMonthPicker"
              v-show="currentView === 'date'"
              role="button"
              class="sw-date-picker__header-label"
              :class="{ active: currentView === 'month' }">{{t(`sw.datepicker.month${ month + 1 }`)}}</span>
            <button
              type="button"
              @click="nextYear"
              :aria-label="t(`sw.datepicker.nextYear`)"
              class="sw-picker-panel__icon-btn sw-date-picker__next-btn sw-icon-d-arrow-right">
            </button>
            <button
              type="button"
              @click="nextMonth"
              v-show="currentView === 'date'"
              :aria-label="t(`sw.datepicker.nextMonth`)"
              class="sw-picker-panel__icon-btn sw-date-picker__next-btn sw-icon-arrow-right">
            </button>
          </div>

          <div class="sw-picker-panel__content">
            <date-table
              v-show="currentView === 'date'"
              @pick="handleDatePick"
...
<script type="text/babel">
  ...
  export default {
    ...
    components: {
      TimePicker, YearTable, MonthTable, DateTable, SwInput, SwButton
    },
    ...
    computed: {
      ...
      yearLabel() {
        const yearTranslation = this.t('sw.datepicker.year');
        if (this.currentView === 'year') {
