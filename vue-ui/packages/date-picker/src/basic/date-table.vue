<template>
  <table
    ...
    <tbody>
    <tr>
      <th v-if="showWeekNumber">{{ t('sw.datepicker.week') }}</th>
      <th v-for="(week, key) in WEEKS" :key="key">{{ t('sw.datepicker.weeks.' + week) }}</th>
    </tr>
    <tr
      class="sw-date-table__row"
...
<script>
  import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, prevDate, nextDate, isDate, clearTime as _clearTime} from 'seawave-ui/src/utils/date-util';
  import Locale from 'seawave-ui/src/mixins/locale';
  import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from 'seawave-ui/src/utils/util';
