...
import { isFunction } from 'seawave-ui/src/utils/types';
  ...
  methods: {
    ...
    getRows() {
      const children = ((this.$slots.default || []).filter(vnode => vnode.tag &&
            vnode.componentOptions && vnode.componentOptions.Ctor.options.name === 'SwDescriptionsItem'));
      ...
      <div class="sw-descriptions">
        {
          (title || extra || $slots.title || $slots.extra)
            ? <div class="sw-descriptions__header">
              <div class="sw-descriptions__title">
                { $slots.title ? $slots.title : title}
              </div>
              <div class="sw-descriptions__extra">
        ...
        <div class="sw-descriptions__body">
          <table class={[{'is-bordered': border}, descriptionsSize ? `sw-descriptions--${descriptionsSize}` : '']}>
            {rows.map(row => (
